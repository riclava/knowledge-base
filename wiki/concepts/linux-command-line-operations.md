...
tags: [concept, linux, command-line, operations, system-administration, workflow, centos, yum, repository, docker, containers, kernel, networking, samba, smb, file-sharing]
---

Linux command-line operations 是通过一组可组合的小命令来检查、修改和验证系统状态的实践，核心对象包括文件、文本、进程、网络、权限、日志、软件源配置、共享服务配置和容器宿主机诊断。
...
- 目前只补上了一个 CentOS 遗留 repo 恢复案例、一个 Docker 网络兼容性案例和一个 Samba 最小共享案例，仍缺少更系统的软件安装升级、远程登录、安全审计或容器编排文档。
