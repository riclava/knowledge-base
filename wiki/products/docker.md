...
created: 2026-04-15
updated: 2026-04-15
sources: [CentOS7离线安装docker问题排查.md, CentOS7升级内核.md, 基于docker构建ubuntu20.04开发环境.md]
tags: [product, docker, containers, centos, ubuntu, linux, container-networking, kernel, elrepo, troubleshooting, development-environment]
---

Docker 是一个容器运行时与镜像分发工具链；在当前知识库里，它既以"安装成功但宿主机网络能力不兼容时仍会运行异常"的运维对象出现，也以"快速构建隔离开发环境"的开发者工具出现。
...
## Containerized Development Environment Use Case

新来源 `[[ubuntu2004-docker-dev-environment-setup]]` 展示了 Docker 的另一种使用模式：作为开发环境容器化工具。

### Pattern

```bash
# 启动长期运行的开发环境容器
docker run -d --name=ubuntu20 --restart=unless-stopped ubuntu:20.04 sleep infinity
...
| 技术 | 说明 |
|---|---|
| `sleep infinity` | 作为主进程保持容器存活，而非直接运行交互式 shell |
| `--restart=unless-stopped` | Docker 重启后自动恢复容器 |
| `docker exec -it` | 按需进入已运行的容器 |
...
详见 [[containerized-development-environment]]。
...
- 开发环境容器化场景还没有覆盖数据卷挂载、端口映射、Dockerfile 构建或 IDE 远程开发集成。
